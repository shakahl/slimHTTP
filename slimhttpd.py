			...
			if self.ssl:
				try:
					...
					if e.errno == 1 and 'SSLV3_ALERT_CERTIFICATE_UNKNOWN' in e.args[1]:
						pass
			log('Accepting new client: {addr}'.format(**{'addr' : na[0]}), level=3, origin='slimHTTP', function='http_serve')
			ns_fileno = ns.fileno()
...
def get_file(root, path, headers={}, *args, **kwargs):
	...
	if b'range' in headers:
		...
		log('Limiting to range: {}-{}'.format(start, stop), origin='slimHTTP', function='get_file')
	else:
	...
	if isfile(real_path):
		...
		log('Returning file content: {} (actual size: {})'.format(len(data), filesize), level=4, origin='slimHTTP', function='get_file')
		return real_path, filesize, data

	log('404 - Could\'t locate file', level=4, origin='slimHTTP', function='get_file')
...
class http_request():
	def __init__(self, client):
		...
		log('Setting up a parser for client: {}'.format(client), once=True, level=4, origin='slimHTTP', function='http_request')
	...
	def parse(self):
		...
		if b'\r\n\r\n' in self.client.data:
			...
			if b'upgrade' in self.headers and b'connection' in self.headers and \
				...
				log('Can\'t handle {} method.'.format(method), once=True, level=1, origin='slimHTTP', function='parse')
		else:
